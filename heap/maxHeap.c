...
int main(){
	...
	printf("\n=== Find 3 smallest data in the array ===\n");
	maxHeap* heap = createHeap(3); 	// find 3 smallest data in the array
	for(int i = 0; i < sizeof(arr)/sizeof(int); i++){
		if(arr[i] < heap->array[0] || heap->count < heap->capacity){  // if data < maxHeap[0] or count < capacity => insert into maxHeap
			insert(heap, arr[i]);
			printArray(heap->array, heap->count);
		}
	}
